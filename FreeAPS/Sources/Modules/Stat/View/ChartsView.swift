...
struct ChartsView: View {
    ...
    var tirChart: some View {
        ...
        .chartForegroundStyleScale([
            NSLocalizedString(
                "Low",
                comment: ""
            ) + " (≤\(low.formatted(.number.grouping(.never).rounded().precision(.fractionLength(1)))))": .pink,
    ...
    var standingTIRchart: some View {
        ...
        .chartForegroundStyleScale([
            ...
            "> \(low.formatted(.number.precision(.fractionLength(fraction)))) - < \(high.formatted(.number.precision(.fractionLength(fraction))))": Color("LoopGreen"),
            NSLocalizedString(
                "High",
                comment: ""
            ) + " (≥ \(high.formatted(.number.grouping(.never).rounded().precision(.fractionLength(1)))))": .orange
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            if !mapGlucose.isEmpty {
                ...
                HStack {
                    ...
                    Text(value.formatted()).foregroundColor(Color("loopGreen"))
                    Text("%").foregroundColor(.secondary)
                }.font(.caption)
                HStack {
